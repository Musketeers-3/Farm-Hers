...
// ─── TYPES ───────────────────────────────────────────────────────────────────
type ThemeKey = "sunny" | "clear_night" | "rainy" | "snowy" | "cloudy" | "partlycloudy" | "stormy";
...
function getRainAlert(condition: ThemeKey, rain: number, pop: number) {
  if (condition === "stormy")             return { text: "⛈️ Thunderstorm alert — stay indoors",   color: "bg-purple-900/40 text-purple-200" };
  if (condition === "rainy" && rain > 15) return { text: "⚠️ Heavy rain alert",                    color: "bg-red-900/40 text-red-200" };
  if (condition === "rainy" && pop >= 70) return { text: "🌧️ High chance of rain today",            color: "bg-blue-900/40 text-blue-200" };
  if (pop >= 40)                          return { text: `🌦️ ${pop}% rain chance — keep watch`,     color: "bg-sky-900/40 text-sky-200" };
  return null;
}

function getIrrigationTip(condition: ThemeKey, temp: number, uv: number, hour: number): string {
...
function StormIcon({ size = 24 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 36" fill="none">
      <path d="M32 20H10a8 8 0 010-16 8 8 0 0115.3-2A6 6 0 1132 20z" fill="rgba(255,255,255,0.55)" />
      <polyline points="22,22 17,30 21,30 16,38" stroke="#FFD600" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
    </svg>
  );
}
...
export function WeatherWidget() {
  ...
  const themeKey        = weather?.themeKey ?? (isNightFallback ? "clear_night" : "sunny");
  ...
  const condition      = weather?.condition      ?? (isNightFallback ? "clear_night" : "sunny");
  const conditionLabel = weather?.conditionLabel ?? (isNightFallback ? "Clear Night" : "Sunny");
  const forecast       = weather?.forecast ?? [];

  const rainAlert     = getRainAlert(condition, forecast[0]?.rain ?? 0, forecast[0]?.pop ?? 0);
  const irrigationTip = getIrrigationTip(condition, temp ?? 25, uvIndex, currentHour);

  if (!mounted) return <div className="h-[170px] w-full rounded-[20px] bg-gray-800 animate-pulse" />;

  if (locationError) return (
    <div className="w-full rounded-[20px] bg-gradient-to-br from-[#1c1c2e] via-[#2a1a2e] to-[#0d0d1e] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] p-5 flex flex-col items-center justify-center gap-3 text-center" style={{ minHeight: 170 }}>
      <span className="text-3xl">🌍</span>
      <div>
        <p className="text-[13px] font-black text-white/90">Location not found</p>
        <p className="text-[11px] text-white/45 mt-0.5">
          <span className="text-white/70 font-semibold">"{userLocation}"</span> didn't match any city.
        </p>
        <p className="text-[10px] text-white/35 mt-1">Try a nearby city or check the spelling.</p>
      </div>
    </div>
  );
  ...
  if (fetchError) return (
    <div className="w-full rounded-[20px] bg-gradient-to-br from-[#1c1c2e] via-[#2a1a2e] to-[#0d0d1e] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] p-5 flex flex-col items-center justify-center gap-3 text-center" style={{ minHeight: 170 }}>
      <span className="text-3xl">⚡</span>
      <div>
        <p className="text-[13px] font-black text-white/90">Weather temporarily unavailable</p>
        <p className="text-[11px] text-white/45 mt-0.5">Could not reach weather service for</p>
        <p className="text-[11px] text-white/70 font-semibold mt-0.5">"{userLocation}"</p>
        <p className="text-[10px] text-white/30 mt-1">Check your internet connection and try again.</p>
      </div>
    </div>
  );
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            {Array.from({ length: 18 }).map((_, i) => (
              <motion.div key={i} className="absolute w-0.5 h-0.5 bg-white rounded-full"
                style={{ left: `${(i * 17 + 5) % 97}%`, top: `${(i * 13 + 3) % 55}%` }}
                animate={{ opacity: [0.2, 1, 0.2] }}
                transition={{ duration: 1.5 + (i % 4) * 0.5, repeat: Infinity, delay: (i % 5) * 0.4 }} />
            ))}
          </div>
        )}
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-15">
            {Array.from({ length: 12 }).map((_, i) => (
              <motion.div key={i} className="absolute w-px bg-blue-300"
                ...
                animate={{ top: ["0%", "110%"] }}
                transition={{ duration: 0.6 + (i % 3) * 0.15, repeat: Infinity, delay: (i % 6) * 0.1, ease: "linear" }} />
            ))}
          </div>
        )}
        ...
        <div className="relative z-10 p-2.5 flex flex-col gap-1.5">
          ...
          <div className="grid grid-cols-4 gap-1 text-[8px] opacity-70 border-t border-white/10 pt-1.5">
            <span className="flex flex-col items-center gap-0.5">
              ...
              <span>{windSpeed != null ? windSpeed : "—"}<br/>km/h</span>
            </span>
            <span className="flex flex-col items-center gap-0.5">
              <Droplets size={9} className="text-blue-200" />
              <span>{humidity != null ? `${humidity}%` : "—"}<br/>hum</span>
            </span>
            <span className="flex flex-col items-center gap-0.5">
              <Eye size={9} className="text-sky-200" />
              <span>{visibility != null ? visibility : "—"}<br/>km vis</span>
            </span>
            <span className="flex flex-col items-center gap-0.5">
              <Thermometer size={9} className="text-orange-300" />
              <span>UV {uvIndex}<br/>{uvIndex >= 8 ? "High" : uvIndex >= 4 ? "Mod" : "Low"}</span>
            </span>
          </div>
          ...
          <div className="grid grid-cols-4 gap-1 border-t border-white/10 pt-1.5">
            {forecast.map((f, i) => (
              <div key={i} className="flex flex-col items-center gap-0.5 rounded-xl bg-white/[0.08] py-1.5 px-1">
