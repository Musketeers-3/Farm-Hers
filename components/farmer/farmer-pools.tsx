...
const GLASS_INPUT =
  "w-full bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-white/50 dark:border-white/10 rounded-2xl px-4 py-3 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all";

export default function FarmerPools() {
  const router = useRouter();

  // ⚡ Syncing directly to your global store
    ...
    <div className="min-h-screen pb-28 lg:pb-8 overflow-x-hidden relative bg-[linear-gradient(135deg,#dcfce7_0%,#dcfce7_20%,#bfdbfe_100%)] dark:bg-none dark:bg-slate-950 transition-colors duration-500">
      ...
      <header className="sticky top-0 z-40 transition-colors duration-300 bg-white/25 dark:bg-slate-950/50 backdrop-blur-[20px] border-b border-white/30 dark:border-white/10">
        ...
        <div className="max-w-5xl mx-auto px-4 pb-4">
          <div className="flex p-1 rounded-2xl bg-white/40 dark:bg-slate-800/40 backdrop-blur-md border border-white/50 dark:border-white/10">
        ...
        <AnimatePresence mode="wait">
                  ...
                  {buyerRequestPools.map((pool) => {
                    ...
                    return (
                      <div
                        key={pool.id}
                        className={cn(
                          "rounded-[24px] p-5 hover:border-emerald-500/50 transition-colors group",
                          GLASS_CLASSES,
                        )}
                      >
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex gap-3 items-center">
                            <div className="w-12 h-12 rounded-xl bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center border border-emerald-200 dark:border-emerald-800/50">
                        ...
                        <div className="space-y-3 mb-5">
                          <div className="flex justify-between text-sm font-bold text-slate-700 dark:text-slate-300">
                            <span>
                              Filled: {pool.filledQuantity || 0} {pool.unit}
                            </span>
                            <span>
                        ...
                        <button
                          ...
                          className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-xl text-sm font-bold shadow-md shadow-emerald-500/20 transition-all flex items-center justify-center gap-2"
                        >
              ...
              {loading ? (
                ...
                >
                  ...
                  <button
                    ...
                    className="mt-4 px-6 py-2 bg-emerald-500 text-white rounded-full font-bold text-sm shadow-md"
                  >
                    Create One Now
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {myPools.map((pool) => (
                    <div
                      key={pool.id}
                      className={cn("rounded-[24px] p-5", GLASS_CLASSES)}
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div>
                      ...
                      <div className="flex items-center gap-4 text-sm font-medium text-slate-600 dark:text-slate-300 bg-white/40 dark:bg-slate-800/40 p-3 rounded-xl border border-white/50 dark:border-white/10">
                        <div className="flex items-center gap-1.5">
                          <Target className="w-4 h-4 text-slate-400" />
                          <span>
                            {pool.filledQuantity}/{pool.targetQuantity}{" "}
                            {pool.unit}
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5 border-l border-slate-300 dark:border-slate-600 pl-4">
          ...
          {tab === "create" && (
            ...
            >
              <div
                className={cn(
                  "rounded-[32px] p-6 sm:p-8 space-y-6",
                  GLASS_CLASSES,
                )}
              >
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">
                    Group your harvest with neighbors to unlock premium
                    enterprise buyer rates.
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full h-14 bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-500/50 text-white rounded-2xl font-bold shadow-lg shadow-emerald-500/20 transition-all flex items-center justify-center gap-2"
                  >
                    {actionLoading ? (
      ...
      <AnimatePresence>
        {joining.pool && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/80 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4"
          >
            <motion.div
            ...
            >
              <div className="text-center mb-6">
                <div className="w-16 h-16 rounded-2xl bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center mx-auto mb-4 shadow-inner">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  Target: {joining.pool.targetQuantity} {joining.pool.unit} •
                  Current: {joining.pool.filledQuantity || 0}
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      className={cn(
                        GLASS_INPUT,
                        "text-center text-2xl font-black py-4",
                      )}
                      type="number"
                      placeholder="0"
                      value={joining.qty}
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 py-4 rounded-2xl text-sm font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 py-4 bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-500/50 text-white rounded-2xl text-sm font-bold shadow-lg shadow-emerald-500/20 transition-all flex justify-center items-center gap-2"
                  >
                    {actionLoading ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
