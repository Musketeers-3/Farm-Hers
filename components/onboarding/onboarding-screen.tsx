...
import { ArrowRight, Sprout, Users, TrendingUp, Shield } from "lucide-react"
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-primary">
            {language === "hi" ? "भारत के किसानों के लिए" : 
             language === "pa" ? "ਭਾਰਤ ਦੇ ਕਿਸਾਨਾਂ ਲਈ" : 
        ...
        <h1 className="text-4xl md:text-5xl font-serif font-bold text-foreground leading-tight text-balance">
          {language === "hi" ? (
            ...
            <>The New Era of <span className="text-gradient-green">Agriculture</span></>
          )}
        </h1>
          ...
          >
            ...
            <div>
              <h3 className="font-semibold text-foreground text-sm">
                {language === "hi" ? feature.titleHi : 
                 language === "pa" ? feature.titlePa : 
                 feature.title}
              </h3>
      ...
      <div className="absolute top-1/4 left-4 glass-card px-3 py-2 rounded-full animate-float">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-foreground">Growth: 12 cm</span>
        </div>
      </div>

      <div className="absolute top-1/3 right-4 glass-card px-3 py-2 rounded-full animate-float" style={{ animationDelay: "1s" }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-foreground">Moisture: 75%</span>
        </div>
      </div>
    </div>
  )
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-2 text-center">
        <h2 className="text-2xl font-serif font-bold text-foreground">
          {language === "hi" ? "आप कौन हैं?" : 
           language === "pa" ? "ਤੁਸੀਂ ਕੌਣ ਹੋ?" : 
           "Who are you?"}
        </h2>
        <p className="text-muted-foreground">
          {language === "hi" ? "अपनी भूमिका चुनें" : 
           language === "pa" ? "ਆਪਣੀ ਭੂਮਿਕਾ ਚੁਣੋ" : 
      ...
      <div className="grid grid-cols-2 gap-4">
        <RoleCard
          title={language === "hi" ? "किसान" : language === "pa" ? "ਕਿਸਾਨ" : "Farmer"}
          desc={language === "hi" ? "अपनी फसल बेचें" : language === "pa" ? "ਆਪਣੀ ਫਸਲ ਵੇਚੋ" : "Sell your crops"}
          image="/images/hero-farmer.jpg"
          onClick={() => onSelect("farmer")}
        />
        <RoleCard
          title={language === "hi" ? "खरीदार" : language === "pa" ? "ਖਰੀਦਦਾਰ" : "Buyer"}
          desc={language === "hi" ? "उत्पाद खरीदें" : language === "pa" ? "ਉਤਪਾਦ ਖਰੀਦੋ" : "Purchase produce"}
